  ...
  return (
    <div className="fixed bottom-8 right-8 z-[100] flex flex-col items-end gap-4 font-sans">
                ...
                >
                  <div className={`flex gap-3 max-w-[85%] ${msg.sender === "user" ? "flex-row-reverse" : "flex-row"}`}>
                    ...
                    <div className={`p-4 rounded-3xl text-sm font-medium leading-relaxed ${
                      msg.sender === "user" 
                        ? "bg-teal-600 text-white rounded-br-none shadow-md shadow-teal-500/10" 
              ...
              {isTyping && (
                <div className="flex justify-start">
                  <div className="flex gap-3 max-w-[85%]">
                    <div className="w-8 h-8 rounded-xl bg-teal-100 text-teal-600 flex items-center justify-center shrink-0 mt-auto">
                      <Bot size={16} />
                    </div>
                    <div className="bg-white dark:bg-slate-800 p-4 rounded-3xl rounded-bl-none shadow-sm flex items-center gap-1.5 border border-slate-100 dark:border-slate-700">
                       <div className="w-1.5 h-1.5 bg-teal-400 rounded-full animate-bounce [animation-duration:0.6s]" />
                       <div className="w-1.5 h-1.5 bg-teal-500 rounded-full animate-bounce [animation-duration:0.6s] [animation-delay:0.1s]" />
                       <div className="w-1.5 h-1.5 bg-teal-600 rounded-full animate-bounce [animation-duration:0.6s] [animation-delay:0.2s]" />
                    </div>
                  </div>
                </div>
              )}
            ...
            <div className="p-4 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800">
              <form onSubmit={handleSend} className="flex gap-2 items-center bg-white dark:bg-slate-800 p-2 rounded-2xl border border-slate-200 dark:border-slate-700 focus-within:border-teal-300 dark:focus-within:border-teal-600 transition-all">
                ...
                <button
                  ...
                  className="w-10 h-10 gradient-primary text-white rounded-xl flex items-center justify-center transition-all shadow-md shadow-teal-500/20 disabled:opacity-50 disabled:shadow-none active:scale-95"
                >
                  <Send size={18} />
                </button>
              </form>
      ...
      <motion.button
        ...
        className="w-16 h-16 gradient-primary text-white rounded-3xl flex items-center justify-center shadow-2xl shadow-teal-500/40 hover:shadow-teal-500/60 transition-all border-none relative group"
        ...
        <AnimatePresence mode="wait">
          {isOpen ? (
            ...
            <motion.div key="msg" initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.5, opacity: 0 }} className="relative">
              ...
              <div className="absolute -top-1 -right-1 w-3 h-3 bg-red-500 border-2 border-teal-600 rounded-full" />
            </motion.div>
          )}
        </AnimatePresence>
      </motion.button>
