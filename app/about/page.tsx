    ...
    <div className="min-h-screen bg-white dark:bg-black font-sans">
      ...
      <section className="pt-40 pb-24 px-6 relative overflow-hidden text-center border-b border-zinc-100 dark:border-zinc-800">
         ...
         <div className="max-w-4xl mx-auto space-y-8">
            ...
            <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-4xl md:text-6xl lg:text-7xl font-black text-zinc-900 dark:text-white tracking-tighter italic leading-tight">
               Architecting the <span className="text-transparent bg-clip-text bg-gradient-to-r from-teal-600 to-cyan-500">Future of Knowledge</span>
            </motion.h1>
      ...
      <section className="py-40 px-6 relative overflow-hidden bg-zinc-950 text-white text-center">
         ...
         <div className="max-w-3xl mx-auto relative z-10 space-y-12">
            <h2 className="text-3xl md:text-5xl font-black tracking-tighter italic leading-tight">Ready to Initialize Your <span className="text-teal-400">Mastery Sequence</span>?</h2>
            <div className="flex flex-col md:flex-row items-center justify-center gap-6">
               <Link href="/register" className="px-10 py-5 bg-white text-black rounded-[2.5rem] font-black text-xs uppercase tracking-[0.3em] italic shadow-2xl hover:bg-teal-500 hover:text-white transition-all w-full md:w-auto">
...
const ValueShard = ({ icon, title, desc }: any) => (
   <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} className="p-12 bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 rounded-[4rem] hover:border-teal-500/20 shadow-xl shadow-zinc-200/5 transition-all group">
      <div className="w-20 h-20 bg-teal-600 rounded-[2rem] flex items-center justify-center text-white mb-8 shadow-xl shadow-teal-600/20 group-hover:rotate-6 transition-all">
         {icon}
      </div>
...
const StatItem = ({ value, label }: any) => (
   <div className="space-y-2">
      ...
      <p className="text-[10px] font-black uppercase tracking-[0.4em] text-teal-600 italic">{label}</p>
   </div>
);
