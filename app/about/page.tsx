    ...
    <div className="min-h-screen bg-white dark:bg-black font-sans">
      ...
      <section className="pt-40 pb-24 px-6 relative overflow-hidden text-center border-b border-zinc-100 dark:border-zinc-800">
         ...
         <div className="container-max space-y-8">
            <span className="text-sm font-medium text-slate-600 dark:text-slate-400 tracking-wide">Institutional Mission</span>
            <h1 className="text-3xl md:text-4xl font-black text-slate-800 dark:text-white tracking-tighter leading-[0.9] mb-2">Empowering the Next Generation of Scholars</h1>
            <p className="text-lg font-medium text-slate-500 dark:text-slate-400 max-w-xl leading-relaxed mx-auto">Providing a premium educational ecosystem designed for academic mastery and professional excellence.</p>
         </div>
      </section>
      ...
      <section className="py-40 px-6 relative overflow-hidden bg-zinc-950 text-white text-center">
         ...
         <div className="max-w-3xl mx-auto relative z-10 space-y-12">
            <h2 className="section-title text-white tracking-tighter leading-tight">Ready to Begin Your <span className="text-teal-400">Success Journey</span>?</h2>
            <div className="flex flex-col md:flex-row items-center justify-center gap-6">
                <Link href="/register" className="px-10 py-5 bg-white text-black rounded-2xl font-black text-xs uppercase tracking-[0.3em] shadow-2xl hover:bg-teal-500 hover:text-white transition-all w-full md:w-auto">
                  Start Learning Now
               </Link>
                <Link href="/courses" className="px-10 py-5 bg-zinc-900 border border-zinc-800 text-zinc-400 rounded-2xl font-black text-xs uppercase tracking-[0.3em] hover:text-white transition-all w-full md:w-auto">
                  Explore Catalog
               </Link>
            </div>
         </div>
...
const ValueShard = ({ icon, title, desc }: any) => (
   <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} className="p-12 bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 rounded-[3rem] hover:border-teal-500/20 shadow-xl shadow-zinc-200/5 transition-all group">
      <div className="w-20 h-20 bg-teal-600 rounded-2xl flex items-center justify-center text-white mb-8 shadow-xl shadow-teal-600/20 group-hover:rotate-6 transition-all">
       ...
       <h3 className="text-xl font-black text-zinc-900 dark:text-white tracking-tighter mb-4">{title}</h3>
       <p className="text-base font-medium text-zinc-600 dark:text-zinc-300 leading-relaxed">{desc}</p>
   </motion.div>
);

const StatItem = ({ value, label }: any) => (
   <div className="space-y-2">
       <h4 className="text-4xl md:text-6xl font-black text-zinc-900 dark:text-white tracking-tighter leading-none">{value}</h4>
